...
void TS_Init(void)
{
	...
	if (status != TS_OK)
	{
		...
		BSP_LCD_DisplayStringAtLine(5, (uint8_t *)"ERROR");
		BSP_LCD_DisplayStringAtLine(6, (uint8_t *)"Touchscreen cannot be initialized");
	}
}
...
void SDCardView()
{
	...
	NetViewActive = false;
	SDCardActive = true;
	ShowSDDirectories((uint8_t *)"SD Karte");
}
...
void BSP_Config(void)
{
	...
	BSP_LCD_LayerDefaultInit(1, LCD_FB_START_ADDRESS+(4*(BSP_LCD_GetXSize()*BSP_LCD_GetYSize())));
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
	  ...
	  else if(!TS_State.touchDetected && HoldTouch)
	  {
		  ReactOnTouch();
		  HoldTouch = false;
	  }
